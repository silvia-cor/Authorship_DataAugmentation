...
class Generator_embed:
    ...
    def generate(self, author_samples, Cl_nn):
        author_sample = tokenizer.encode(np.random.choice(author_samples), return_tensors="pt")[0]
        with torch.no_grad():
            generated_sample = Cl_nn.model.embedding(torch.unsqueeze(author_sample[:5], dim=0).to(self.device))
    ...
    def prepare_for_LMtraining(self, texts, Cl_nn):
        lm_inputs, lm_targets = get_examples_for_LM(texts, tokenizer)
        with torch.no_grad():
            embed_inputs = [Cl_nn.model.embedding(lm_input.to(self.device)).detach().clone().cpu().float()
                            for lm_input in lm_inputs]
            embed_targets = [Cl_nn.model.embedding(lm_target.to(self.device)).detach().clone().cpu().float()
                             for lm_target in lm_targets]
        return embed_inputs, embed_targets
