...
# divide dataset into training, validation and test sets (original texts, pos_texts and labels)
...
# victoria dataset
# very simple dataset from '800 English novels
def process_victoria(data_path='../dataset/victoria/Gungor_2018_VictorianAuthorAttribution_data-train.csv', **kwargs):
...
def process_tweepfake(data_path='../dataset/TweepFake/', **kwargs):
...
# EBG-obfuscation
def process_ebg(data_path='../dataset/EBG-obfuscation/', **kwargs):
    texts, labels = [], []
...
# RJ-obfuscation
def process_rj(data_path='../dataset/RJ-obfuscation/', **kwargs):
    texts, labels = [], []
    for file in os.listdir(data_path + 'train'):
        with open(data_path + 'train/' + file, 'r') as data_file:
            splits = get_splits(data_file.read())
...
def process_pan11(data_path='../dataset/pan11/', **kwargs):
