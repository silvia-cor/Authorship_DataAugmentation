...
class CustomDataloader:
    def __init__(self, data, feats_extractor, batch_size, shuffle=True, labels_long=True, onehot=False,
                 nn_embed_path=None):
        self.s = True if all(isinstance(s, str) for s in data[0]) else False
        if not self.s:
            if nn_embed_path is not None:
                feats_extractor.model.load_state_dict(torch.load(nn_embed_path))
    ...
    def _collate_fn(self, batch):
        ...
        if self.s:
            final_samples = self.tokenizer(samples, return_tensors="pt", padding=True)["input_ids"]
            if self.onehot:
        ...
        if self.bf_extractor is not None:
            ...
            return final_samples, bf, labels
        else:
            return final_samples, None, labels


class BertDataloader:
    def __init__(self, data, tokenizer, batch_size, labels_long=True, shuffle=True):
